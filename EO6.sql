...
SELECT
	...
	/*CONCAT and + don't operate the same. 
	If any expression has the value NULL when using +,
	then NULL will be returned. For CONCAT, it will implicitly replace NULL with '', so NULL won't be returned. */
	user_email_id,
	created_ts,
	YEAR(created_ts) AS created_year
FROM
...
SELECT 
    SUBSTRING(REPLACE(user_phone_no, '+', ''), 1, CHARINDEX(' ', REPLACE(user_phone_no, '+', '')) - 1) AS country_code,
    COUNT(*) AS user_count
FROM 
    users
WHERE 
...
GROUP BY 
    SUBSTRING(REPLACE(user_phone_no, '+', ''), 1, CHARINDEX(' ', REPLACE(user_phone_no, '+', '')) - 1)
ORDER BY 
    CAST(SUBSTRING(REPLACE(user_phone_no, '+', ''), 1, CHARINDEX(' ', REPLACE(user_phone_no, '+', '')) - 1) AS INT);
...
SELECT 
    COUNT(*) AS invalid_subtotals_count
FROM 
    order_items
WHERE 
...
FROM
    orders
WHERE
    order_date >= '2014-01-01' AND order_date < '2014-02-01'
GROUP BY
    CASE 
        WHEN DATEPART(dw, order_date) IN (1, 7) THEN 'Weekend days'
        ELSE 'Week days'
